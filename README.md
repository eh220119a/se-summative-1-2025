...
4. Refactoring & Improving the Code
    ...
    Once a test passed, I refactored the code while ensuring all tests continued to pass. This allowed us to optimise performance and maintain readability without breaking functionality.
...
##  Project Evaluation
The stand-up scheduler application successfully streamlined the process of assigning facilitators for daily Agile stand-ups, ensuring fair and structured scheduling. By implementing a two-week rotation system, the application prevented the same individuals from being overburdened while maintaining transparency within the team. The UI closely followed the initial Figma prototype, offering a modern and structured layout that separated stand-up scheduler setup from the schedule display. The notification bar effectively displayed the assigned stand-up host, reducing confusion and improving team coordination. Additionally, the project benefited from Test-Driven Development (TDD), where Jest was used to validate key functions like getFormattedToday() and displayTodaysHost(). Writing tests before implementation improved code reliability and maintainability, while GitHub’s Kanban board system facilitated efficient task tracking and version control.

Despite these successes, there are several areas for improvement that could enhance the application’s functionality and usability. Moving fully onto Node.js would allow direct file storage and retrieval, eliminating the need for manual CSV downloads and local storage reliance. Additionally, integrating GitHub Actions for automated Jest tests and implementing continuous integration (CI) would further streamline development. Improved error handling, such as preventing duplicate name entries and ensuring properly formatted CSV uploads, would enhance user experience. Lastly, deploying the application on a cloud-based platform like Vercel or Heroku would enable remote access, making it easier for teams to manage their stand-up schedules without requiring local setup. While the project achieved its core objectives, these refinements would make it a fully automated, scalable tool for Agile teams.